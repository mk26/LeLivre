	...
	<tfoot>
		<tr class="info">
			...
			<td class="text-success"><? if($amtprice!=0) echo "$ ".$amtprice; ?></td>
				...
				<form class="form-horizontal" role="form" action="orderreceipt.php" method="post">
	    <? getAllCards();?>
	    <br><hr>
		  <div class="col-sm-10">

	    <button type="submit" class="btn btn-primary btn-lg" name="checkout" <? if($_SESSION['cartqty']==0 || $results==NULL) echo "disabled";?>><span class="glyphicon glyphicon-chevron-right"></span>&nbsp;&nbsp;Continue to Checkout</button>
...
function getAllCards()
{
    ...
    else
    {
		...
		{
			echo "<input type=\"radio\" name=\"cards[]\" value=\"".$row['card_no']."\" checked>&nbsp;&nbsp;".$row['card_no'].$row['exp_date'].$row['type']."<br>";
		}
	}
}
?>
