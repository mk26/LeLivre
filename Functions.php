...
/*
get average score for a book; return a score;
*/
...
function completeTransaction($username, $card_no)
{
    ...
    for ($i = 0; $i < sizeof($items); $i++) {
        $sql3 = "SELECT price FROM book WHERE isbn = $items[$i][0]";
        echo '$item[$i][0]';
        $result3 = pg_query($sql3);
        $unit_price = pg_fetch_row($result3);
        echo "unit"." ".$unit_price;
        $checkout_price = $unit_price * $items[$i][1];
        echo "chekcout".$checkout_price;
        //$sql = "INSERT INTO transaction VALUES('$username', $order_id, $items[$i][0], $items[$i][1],
        //(SELECT price FROM book WHERE isbn = $items[$i][0])[0] * $items[$i][1], '4207567887659999', date('y/m/d H:i:s'))";
        //$result = pg_query($sql);
    }
...
//tester
//completeTransaction('jane2', 'sdrwerwe');
